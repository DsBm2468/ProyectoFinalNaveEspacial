// CODIGO QUE SE USO EN LA EXPO Y EL EXAMEN DE REALIDAD MIXTA
using UnityEngine;
...
//INTENTO DE ARREGLO PARA QUE NO TRASPASEN LOS OBJETOS LAS PAREDES (Aun falla)
//using UnityEngine;
//using UnityEngine.UI;
...
//                    bool thereIsAWall = Physics.Raycast(obj.transform.position, directionToSpace, 1.2f); // Se crea un booleano con el que los objetos detectan las paredes para evitar que las traspasen, para ello se aplica un sensor Raycast que proyecta un rayo invisible desde un punto de origen (Posicion del objeto) hacia una dirección especifica (punto de succión) 
//                    // Este inicialmente está en false, utiliza un alcance del laser en metros, se usó un valor de 1.2 para que el laser sobresalga del cuerpo del objeto, un márgen para que el script detecte las paredes antes que la superficie geométrica del objeto la toque y traspase

//                    if (thereIsAWall == false) // Si no hay una pared...
//                    {
//                        obj.transform.position += directionToSpace * intensityForce * Time.deltaTime;

//                        // el objeto se acerca gradualmente (.MoveTowards) hacia el punto de succión
//                        //obj.transform.position = Vector3.MoveTowards(obj.transform.position, suctionPoint.position, intensityForce * Time.deltaTime);
